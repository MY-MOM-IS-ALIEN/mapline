...
public class MapController {
	...
	public String getMapKeywordPoint(
			@RequestParam(required = false) String keyword, 
			@RequestParam(required = false) Double x, // 중심 x 좌표
			@RequestParam(required = false) Double y, // 중심 y 좌표
			Model model) throws IOException, InterruptedException {
		if (keyword != null && !keyword.isEmpty()) {
			Point centerPoint = new Point(x, y);
			List<Point> pointList = KakaoApiUtil.getPointByKeyword(keyword, centerPoint);
