...
public class KakaoApiUtil {
	 ...
	 */
	public static List<Point> getPointByKeyword(String keyword, Point point) throws IOException, InterruptedException {
		HttpClient client = HttpClient.newHttpClient();
		String url = "https://dapi.kakao.com/v2/local/search/keyword.json";
		url += "?query=" + URLEncoder.encode(keyword, "UTF-8");
		url += "&category_group_code=PM9"; // 약국 코드
		url += "&x=" + point.getX();
		url += "&y=" + point.getY();
		...
		for(Document document : documents) {
			Point point1 =new Point(document.getX(), document.getY());
			point1.setName(document.getPlaceName());
			point1.setPhone(document.getPhone());
